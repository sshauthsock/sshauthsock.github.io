import{L as f,E as D,a as k}from"./utils-_hhoRv_V.js";function S(e,i,t={}){if(!e){f.error("[ErrorRecovery] Container not provided");return}const{onRetry:r=null,onGoHome:n=()=>{window.location.href="/"},title:a="오류가 발생했습니다",message:c=null}=t,l=c||D.getUserFriendlyMessage(i?.message||""),s=`error-${Date.now()}`;e.innerHTML=`
  ...
  `;const o=e.querySelector(`#${s}`);if(r){const m=o.querySelector(".error-retry-btn");m&&m.addEventListener("click",()=>{if(f.log("[ErrorRecovery] Retry button clicked"),typeof r=="function")try{r()}catch(u){f.error("[ErrorRecovery] Retry failed:",u),S(e,u,t)}})}const p=o.querySelector(".error-home-btn");p&&p.addEventListener("click",()=>{f.log("[ErrorRecovery] Go home button clicked"),typeof n=="function"&&n()});const v=o.querySelector(".error-reload-btn");if(v&&v.addEventListener("click",()=>{f.log("[ErrorRecovery] Reload button clicked"),window.location.reload()}),!document.getElementById("error-recovery-styles")){const m=document.createElement("style");m.id="error-recovery-styles",m.textContent=`
  ...
  `,c.appendChild(o);const p=d("div","modal-header"),v=d("h3","",{text:t});p.appendChild(v),c.appendChild(p);const m=d("div","modern-chakra-container");c.appendChild(m),G(e,i,m,r,n),a.style.display="flex",document.body.style.overflow="hidden";const u=g=>{g.key==="Escape"&&b()};document.addEventListener("keydown",u),a._escListener=u,P=a,a.addEventListener("click",g=>{g.target===a&&b()}),setTimeout(()=>{try{const g=s.querySelector(".kakao_ad_area"),h=o.querySelector(".kakao_ad_area");window.adfit&&(g&&window.adfit.render(g),h&&window.adfit.render(h))}catch(g){console.error("Kakao AdFit: Error rendering ads in Chak modal:",g)}},100)}function G(e,i,t,r,n){const a=O(e,r);if(Object.keys(a).length===0){t.innerHTML=`
    ...
    `;return}const c=d("div","chakra-results-tabs"),l=d("div","chakra-results-content"),s=$(i);t.appendChild(s),t.appendChild(c),t.appendChild(l),Object.entries(a).forEach(([o,p],v)=>{const m=d("div","chakra-tab",{"data-stat":o,text:o}),u=d("span","chakra-tab-badge",{text:`${p.length}곳`});m.appendChild(u);const g=d("div","chakra-tab-panel",{"data-stat":o});v===0&&(m.classList.add("active"),g.classList.add("active")),T(g,p,i,n),c.appendChild(m),l.appendChild(g),m.addEventListener("click",()=>{c.querySelectorAll(".chakra-tab").forEach(h=>h.classList.remove("active")),l.querySelectorAll(".chakra-tab-panel").forEach(h=>h.classList.remove("active")),m.classList.add("active"),g.classList.add("active")})}),k(t)}function $(e,i,t){const r=F(e),n=d("div","quick-stats-summary"),a=d("div","summary-title");a.innerHTML=`
  ...
  `;const c=d("div","stats-summary-grid");return Object.keys(r).length===0?c.innerHTML='<div style="grid-column: 1/-1; text-align: center; color: #64748b;">적용된 능력치가 없습니다</div>':Object.entries(r).sort((l,s)=>s[1]-l[1]).forEach(([l,s])=>{const o=d("div","summary-stat-item");o.innerHTML=`
        ...
        `,c.appendChild(o)}),n.appendChild(a),n.appendChild(c),n}function T(e,i,t,r){const n=i.reduce((c,l)=>{const s=l.part.split("_")[0];return(c[s]=c[s]||[]).push(l),c},{}),a=d("div","equipment-parts-grid");Object.entries(n).forEach(([c,l])=>{const s=d("div","equipment-part-card"),o=_(l,t);o.fullyUpgraded>0?s.classList.add("fully-upgraded"):o.partiallyUpgraded>0&&s.classList.add("has-upgrades");const p=d("div","equipment-card-header");p.innerHTML=`
    ...
    `;const v=d("div","upgrade-levels-container");l.sort((m,u)=>{const g=parseInt(m.level.replace(/\D/g,""),10),h=parseInt(u.level.replace(/\D/g,""),10);return g-h}).forEach(m=>{const u=`${m.statName}_${m.part}_${m.level}_${m.index}`,g=t[u]||{isUnlocked:!1,level:0},h=d("div","upgrade-level-row");let R="level-unused",I="미강화";g.isUnlocked&&(g.level===3?(R="level-complete",I="완료"):(R="level-partial",I=`${g.level}/3`)),h.innerHTML=`
          <div class="level-indicator ${R}">
            ${m.level}
          </div>
          <div class="level-details">
            <div class="level-stat-info">
              <div class="level-stat-name">${y(m.statName)}</div>
              <div class="level-stat-value">+${m.maxValue}</div>
            </div>
            <div class="level-status-badge status-${R.replace("level-","")}">
              ${I}
            </div>
          </div>
        `,h.addEventListener("click",()=>{r(m.part,m.level),h.style.background="#dbeafe",setTimeout(()=>{h.style.background=""},300)}),v.appendChild(h)}),s.appendChild(p),s.appendChild(v),a.appendChild(s)}),e.appendChild(a)}function _(e,i){let t=0,r=0,n=0;e.forEach(c=>{const l=`${c.statName}_${c.part}_${c.level}_${c.index}`,s=i[l]||{isUnlocked:!1,level:0};s.isUnlocked&&(t++,s.level===3?r++:n++)});const a=Math.round(t/e.length*100);return{upgradedCount:t,fullyUpgraded:r,partiallyUpgraded:n,progressPercentage:a,totalCount:e.length}}function j(e){return{목걸이:"목걸이",반지:"반지",팔찌:"팔찌",벨트:"벨트",신발:"신발",장갑:"장갑"}[e]||e}function F(e,i){const t={};return Object.entries(e).forEach(([r,n])=>{if(!n.isUnlocked||n.level===0)return;const a=r.split("_");if(a.length<4)return;const c=a[0],l=y(c),s=10,o=n.level/3,p=Math.round(s*o);t[l]=(t[l]||0)+p}),t}function O(e,i){const t={};e.constants.parts.forEach(n=>{const a=n.split("_")[0];e.constants.levels.forEach(c=>{const l=`lv${c.replace("+","")}`,s=e.equipment[a]?.[l]||{};let o=0;Object.entries(s).forEach(([p,v])=>{const m=y(p);i.includes(m)&&(t[m]||(t[m]=[]),t[m].push({part:n,level:c,statName:p,maxValue:v,index:o,cardId:`${p}_${n}_${c}_${o}`})),o++})})});const r={};return Object.keys(t).sort().forEach(n=>{r[n]=t[n].sort((a,c)=>{const l=a.part.split("_")[0],s=c.part.split("_")[0];if(l!==s)return l.localeCompare(s);const o=parseInt(a.level.replace(/\D/g,""),10),p=parseInt(c.level.replace(/\D/g,""),10);return o-p})}),r}function b(){P&&(document.removeEventListener("keydown",P._escListener),P.remove(),P=null),document.body.style.overflow="auto"}export{V as E,X as F,J as G,z as P,x as S,W as a,Q as b,d as c,N as d,U as e,H as f,B as g,Z as h,K as i,Y as r,S as s};
